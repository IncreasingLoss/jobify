...
::  - Creates venv, installs deps (ensuring python-jobspy is used), runs setup notebook
...
:: ---------------------------------------------------------------------
:: 2) Check / install Python 3.12 (CRITICAL for numpy compatibility)
:: ---------------------------------------------------------------------
echo.
...
echo [i] Cleaning up any incorrectly installed 'jobspy' package...
python -m pip uninstall jobspy -y >nul 2>&1

if not exist "requirements.txt" (
    echo [i] requirements.txt not found. Creating a default one with correct dependencies...
    (echo python-jobspy) > requirements.txt
...
echo [i] Ensuring nbconvert and ipykernel are installed...
...
if not exist "%NB_PATH%" (
    ...
    echo [i] Running notebook - this may take a minute...
...
if !errorlevel! neq 0 (
    rem Use /D for the working directory to avoid nested-cd quoting issues.
    rem The doubled outer quotes around the cmd /k argument are required by cmd's
    rem quote-stripping rules when the inner command itself contains quotes.
